...
class AthenaQuery():
    ...
    def __init__(self, user_query, database='qcdb'):
        ...
        self.query_output_bucket = config.AlphaVantage.ATHENA_OUTPUT_BUCKET
        self.athena_client = aws_session.client('athena', region_name=config.AlphaVantage.ATHENA_REGION_NAME)
        self.s3_client = aws_session.client('s3', region_name=config.AlphaVantage.ATHENA_REGION_NAME)
    ...
    def init_query(self):
        ...
        response = self.athena_client.start_query_execution(
            ...
            WorkGroup=config.AlphaVantage.ATHENA_WORKGROUP,
        )
        return response
    ...
    def run_query(self):
        ...
        while ((datetime.now() - start_time).total_seconds() < config.AlphaVantage.ATHENA_QUERY_TIMEOUT) \
            ...
            time.sleep(config.AlphaVantage.ATHENA_SLEEP_BETWEEN_REQUESTS)
    ...
    def display_s3_url(self):
        s3_output_url = f'{config.AlphaVantage.S3_OBJECT_ROOT}/{self.query_output_bucket}/{self.output_loc}/{self.execution_id}.csv'
        s3_output_str = f's3 output: {s3_output_url}'
        log.info(s3_output_str)
        print(s3_output_str)

    def display_athena_url(self):
        athena_query_url = f'https://{config.AlphaVantage.ATHENA_REGION_NAME}.console.aws.amazon.com/athena/home?' \
                           f'region={config.AlphaVantage.ATHENA_REGION_NAME}#query/history/{self.execution_id}'
